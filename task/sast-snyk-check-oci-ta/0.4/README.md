...
|SCAN_INPUT_MODE|When set to image-ref-scripts, fetch script-like files from image-url@image-digest (OCI artifact manifest) into /var/workdir/source before the main scan step. Requires use-source. Default source-artifact uses only restored SOURCE_ARTIFACT.|source-artifact|false|
|SCRIPT_PATH_REGEX|Extended regex matching org.opencontainers.image.title paths in the OCI manifest when SCAN_INPUT_MODE=image-ref-scripts.|(^|/)(Dockerfile|Containerfile|[^/]+\.(sh|bash|zsh|ksh|py|rb|pl|js|mjs|cjs|ts|ps1))$|false|
